...
public class VueJoueursClassement extends VerticalLayout {
    ...
    private Grid<Classement.ButeurInfo> gridCurrentTournoi;
    private Grid<Classement.ButeurInfoAllTournois> gridAllTournois;
    ...
    private List<Classement.ButeurInfo> currentButeursCache;
    private List<Classement.ButeurInfoAllTournois> allButeursCache;
    ...
    private Grid<Classement.ButeurInfo> createGrid(boolean showTournoi) {
        Grid<Classement.ButeurInfo> grid = new Grid<>(Classement.ButeurInfo.class, false);
        grid.setSizeFull();
        grid.setPageSize(20);

        grid.addColumn(Classement.ButeurInfo::getPlaceFormatted).setHeader("Place").setWidth("100px").setFlexGrow(0);
        grid.addColumn(Classement.ButeurInfo::getNomJoueur).setHeader("Joueur").setAutoWidth(true);
        grid.addColumn(Classement.ButeurInfo::getNomEquipe).setHeader("Équipe").setAutoWidth(true);
        grid.addColumn(Classement.ButeurInfo::getNombreButs).setHeader("Buts").setWidth("100px").setFlexGrow(0);
    ...
    @SuppressWarnings("deprecation")
    private Grid<Classement.ButeurInfoAllTournois> createAllGrid(boolean showTournoi) {
        Grid<Classement.ButeurInfoAllTournois> grid = new Grid<>(Classement.ButeurInfoAllTournois.class, false);
        grid.setSizeFull();
        grid.setPageSize(20);

        grid.addColumn(Classement.ButeurInfoAllTournois::getPlaceFormatted).setHeader("Place").setWidth("100px").setFlexGrow(0);
        grid.addColumn(Classement.ButeurInfoAllTournois::getNomJoueur).setHeader("Joueur").setAutoWidth(true);
        grid.addColumn(Classement.ButeurInfoAllTournois::getNomEquipe).setHeader("Équipe").setAutoWidth(true);
        grid.addColumn(Classement.ButeurInfoAllTournois::getNombreButs).setHeader("Buts").setWidth("100px").setFlexGrow(0);

        if (showTournoi) {
            grid.addColumn(Classement.ButeurInfoAllTournois::getNomTournoi).setHeader("Tournoi").setAutoWidth(true);
        }
    ...
    private void loadButeursCurrentTournoi() {
        ...
        try (Connection con = ConnectionPool.getConnection()) {
            List<Classement.ButeurInfo> buteurs = Classement.chargerButeursTournoiActuel(con, tournoiId);
    ...
    private void loadButeursAllTournois() {
        try (Connection con = ConnectionPool.getConnection()) {
            List<Classement.ButeurInfoAllTournois> buteurs = Classement.chargerButeursAllTournois(con);
    ...
    private HorizontalLayout buildPodium(List<Classement.ButeurInfo> buteurs, boolean showTournoi) {
        List<Classement.ButeurInfo> first = new ArrayList<>();
        List<Classement.ButeurInfo> second = new ArrayList<>();
        List<Classement.ButeurInfo> third = new ArrayList<>();

        for (Classement.ButeurInfo b : buteurs) {
            switch (b.getPlace()) {
                case 1 -> first.add(b);
                case 2 -> second.add(b);
                case 3 -> third.add(b);
    ...
    private HorizontalLayout buildPodiumAll(List<Classement.ButeurInfoAllTournois> buteurs) {
        List<Classement.ButeurInfoAllTournois> first = new ArrayList<>();
        List<Classement.ButeurInfoAllTournois> second = new ArrayList<>();
        List<Classement.ButeurInfoAllTournois> third = new ArrayList<>();

        for (Classement.ButeurInfoAllTournois b : buteurs) {
            switch (b.getPlace()) {
                case 1 -> first.add(b);
                case 2 -> second.add(b);
                case 3 -> third.add(b);
    ...
    private VerticalLayout createPodiumColumn(String title, List<Classement.ButeurInfo> players, String color, String columnClass) {
        ...
        if (players.isEmpty()) {
            ...
            for (Classement.ButeurInfo player : players) {
    ...
    private VerticalLayout createPodiumColumnAll(String title, List<Classement.ButeurInfoAllTournois> players, String color, String columnClass) {
        VerticalLayout col = new VerticalLayout();
        col.setAlignItems(Alignment.CENTER);
        col.setSpacing(false);
        col.setPadding(false);
        ...
        if (players.isEmpty()) {
            ...
            for (Classement.ButeurInfoAllTournois player : players) {
                Span playerSpan = new Span(player.getNomJoueur());
                playerSpan.addClassName("podium-player-name");
                String tooltip = player.getNomEquipe() + " — " + player.getNombreButs() + " but" + (player.getNombreButs() > 1 ? "s" : "") 
    ...
    private List<Classement.ButeurInfo> onlyScorers(List<Classement.ButeurInfo> src) {
        List<Classement.ButeurInfo> out = new ArrayList<>();
        for (Classement.ButeurInfo b : src) {
            if (b.getNombreButs() > 0) out.add(b);
        }
        return out;
    }

    private List<Classement.ButeurInfoAllTournois> onlyScorersAll(List<Classement.ButeurInfoAllTournois> src) {
        List<Classement.ButeurInfoAllTournois> out = new ArrayList<>();
        for (Classement.ButeurInfoAllTournois b : src) {
            if (b.getNombreButs() > 0) out.add(b);
        }
        return out;
    }
    ...
    private void updateLeaderboardCurrent() {
        ...
        List<Classement.ButeurInfo> scorers = onlyScorers(currentButeursCache);
    ...
    private void updateLeaderboardAll() {
        ...
        List<Classement.ButeurInfoAllTournois> scorers = onlyScorersAll(allButeursCache);
